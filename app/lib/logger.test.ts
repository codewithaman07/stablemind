...
import { sanitizeValue } from './logger.ts';
...
test('sanitizeValue should redact sensitive keys in objects', () => {
  ...
  const sanitized = sanitizeValue(sensitiveObj) as any;
...
test('sanitizeValue should redact nested sensitive keys', () => {
  ...
  const sanitized = sanitizeValue(nestedObj) as any;
...
test('sanitizeValue should handle circular references', () => {
  ...
  const sanitized = sanitizeValue(obj) as any;
...
test('sanitizeValue should handle Error objects', () => {
    ...
    (error as any).apiKey = 'secret-key';

    const sanitized = sanitizeValue(error) as any;
