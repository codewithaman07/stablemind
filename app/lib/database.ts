...
export async function toggleSupport(userId: string, postId: string): Promise<boolean> {
    ...
    if (existing) {
        ...
        const { data: post } = await supabase.from('peer_posts').select('support_count').eq('id', postId).single();
        if (post) {
            await supabase.from('peer_posts').update({ support_count: Math.max(0, (post.support_count || 1) - 1) }).eq('id', postId);
        }
        return false;
    } else {
        // Add support
        ...
        const { data: post } = await supabase.from('peer_posts').select('support_count').eq('id', postId).single();
        if (post) {
            await supabase.from('peer_posts').update({ support_count: (post.support_count || 0) + 1 }).eq('id', postId);
        }
        return true;
    }
}
